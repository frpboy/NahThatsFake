...
function getPlanDetails(planId) {
  const plans = {
    'ind_weekly': { amount: 2900, days: 7, is_credit: false, stars: 150 },
    'ind_monthly': { amount: 9900, days: 30, is_credit: false, stars: 500 },
    'ind_annual': { amount: 79900, days: 365, is_credit: false, stars: 4000 },
    'ind_lifetime': { amount: 199900, days: 36500, is_credit: false },
    'credits_50': { amount: 4900, credits: 50, is_credit: true, stars: 250 },
    'credits_100': { amount: 8900, credits: 100, is_credit: true, stars: 450 },
...
app.post('/api/payment/create-stars-invoice', validateTelegramData, async (req, res) => {
  ...
  const planDetails = getPlanDetails(planId);
  if (!planDetails || !planDetails.stars) {
    return res.status(400).json({ error: 'Invalid plan or star price not available' });
  }
  ...
  try {
    ...
    const response = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/createInvoiceLink`, {
      ...
      body: JSON.stringify({
        ...
        currency: "XTR",
        prices: [{ label: title, amount: planDetails.stars }] // amount in Stars
      })
    });
