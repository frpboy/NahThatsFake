...
## 2024-07-17 - Fix payment replay vulnerability and silent database errors
**Vulnerability:** The Razorpay payment verification endpoint lacked idempotency checks against replay attacks, allowing attackers to replay the same valid payload to duplicate credits. It also silently ignored Supabase database errors on insertion and updates.
**Learning:** Always include an explicit check for `last_payment_id` against the incoming transaction ID on endpoints handling monetary credits. Extract and check the `{ error }` object on all Supabase operations.
**Prevention:** Implement strict idempotency handling and explicit error checking on all financial logic and Supabase operations.
