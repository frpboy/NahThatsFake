...
## 2026-06-18 - Consolidate Screen Reader Announcements for Interactive Cards
**Learning:** For complex interactive items rendered via vanilla JS (like list items or cards) that contain multiple structural/visual children, screen readers often read out the inner elements separately, creating a fragmented and confusing experience for users navigating by keyboard or assistive tech.
**Action:** Consolidate these fragmented announcements by applying a single, descriptive `aria-label` to the outer parent container (the interactive element) and setting `aria-hidden="true"` on all visual/structural child elements inside it. This provides a single, cohesive announcement containing all relevant context at once.
