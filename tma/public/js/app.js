...
function renderChecks() {
    ...
    const html = checks.map(check => {
        ...
        const checkTypeLabel = check.check_type === 'image' ? 'Image Analysis' : 'Link Analysis';
        const ariaLabel = `${checkTypeLabel} on ${date}. Risk level: ${check.risk_level || 'UNKNOWN'}. Score: ${scorePercent}%. Press Enter to view details.`;
        ...
        return `
            <div class="check-item" role="button" tabindex="0" aria-label="${ariaLabel}" onclick="viewCheckDetails('${check.id}')" onkeydown="if(event.key === 'Enter' || event.key === ' ') { event.preventDefault(); viewCheckDetails('${check.id}'); }">
                ...
                <div class="check-details" aria-hidden="true">
                    <div class="check-type">${checkTypeLabel}</div>
