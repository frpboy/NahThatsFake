...
## 2024-06-09 - Consolidating Screen Reader Announcements on Interactive Cards
**Learning:** Complex interactive cards (like the recent checks items) that contain multiple data points (icons, type, date, risk level, percentage) result in fragmented and confusing readouts for screen reader users when focus is applied.
**Action:** Always consolidate announcements on such elements by applying a single descriptive `aria-label` to the parent interactive container (e.g. `role="button"`) and setting `aria-hidden="true"` on all inner visual or structural child elements to prevent redundant readouts.
