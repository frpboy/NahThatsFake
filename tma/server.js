...
function getPlanDetails(planId) {
  const plans = {
    ...
    'ind_lifetime': { amount: 199900, stars: 10000, days: 36500, is_credit: false }, // added hypothetical stars if missing, but fallback should exist
...
app.post('/api/payment/create-stars-invoice', validateTelegramData, async (req, res) => {
  ...
  if (!planDetails || !planDetails.stars) {
    return res.status(400).json({ error: 'Invalid plan or missing stars pricing' });
  }
