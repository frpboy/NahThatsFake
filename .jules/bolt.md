...
## 2025-02-12 - Concurrent Supabase Queries in Handlers
**Learning:** Found multiple independent Supabase queries being executed sequentially using `await supabase...` sequentially in a command handler.
**Action:** When multiple independent database queries are needed, always combine them using `Promise.all` to reduce network round-trips and halve the database wait time.
