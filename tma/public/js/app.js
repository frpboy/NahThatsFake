...
function renderChecks() {
    ...
    const html = checks.map(check => {
        ...
        const typeName = check.check_type === 'image' ? 'Image Analysis' : 'Link Analysis';
        ...
        return `
            <div class="check-item" role="button" aria-label="${typeName} on ${date}. Risk level: ${check.risk_level || 'UNKNOWN'} at ${scorePercent}%." tabindex="0" onclick="viewCheckDetails('${check.id}')" onkeydown="if(event.key === 'Enter' || event.key === ' ') { event.preventDefault(); viewCheckDetails('${check.id}'); }">
                <div class="check-icon ${check.check_type}" aria-hidden="true">${icon}</div>
                <div class="check-details" aria-hidden="true">
                    <div class="check-type">${typeName}</div>
