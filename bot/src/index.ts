...
bot.command('stats', async (ctx) => {
  ...
  // ⚡ Bolt: Combine independent stat queries into a single Promise.all call
  // to fetch data concurrently instead of sequentially, reducing total DB latency
  // by executing both count queries in parallel.
  const [
    { count: users },
    { count: checks }
  ] = await Promise.all([
