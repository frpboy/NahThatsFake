...
## 2026-06-12 - Prevent Replay Attacks and Fallback Bypass in Telegram Auth
**Vulnerability:** The `initData` validation in `validateTelegramData` failed to verify `auth_date`, allowing replay attacks where intercepted payloads could be reused indefinitely. Additionally, it unsafely fell back to validating hashes against a hardcoded `'fallback'` key when `BOT_TOKEN` was unconfigured, essentially bypassing signature verification.
**Learning:** Security middleware must always validate time-based expiration constraints (`auth_date`) on signed payloads. Furthermore, cryptographic operations should strictly fail closed (e.g., throwing a 500 error) when critical secrets like environment variables are missing, rather than applying a graceful but insecure default fallback.
**Prevention:** Explicitly reject incoming payloads missing `auth_date` or those older than 24 hours. Ensure application startup or middleware fails explicitly and securely if `process.env.BOT_TOKEN` is missing, preventing unintended logic bypasses.
