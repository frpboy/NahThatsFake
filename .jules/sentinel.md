...
## 2024-05-23 - Razorpay Webhook Signature Verification Hardening
**Vulnerability:** HMAC signatures for webhooks were computed against `JSON.stringify(req.body)`, which altered formatting and key order, leading to invalid signatures. Additionally, missing environment variables for the secret could cause a `TypeError` and crash the server, resulting in a DoS.
**Learning:** Using `JSON.stringify` modifies the raw payload. Cryptographic functions in Node.js require strict input validation to avoid unhandled exceptions when passing `undefined` secrets or bodies.
**Prevention:** Always capture the exact raw request body using `express.json({ verify: ... })`. Validate the existence of secrets before passing them to `crypto.createHmac()` and provide a fallback string (e.g., `req.rawBody || ''`) when updating the hash.
