...
## 2024-06-06 - Native base64url encoding
**Learning:** In Node.js environments (v14.18+), using `Buffer.from(data).toString('base64url')` is significantly more efficient for generating URL-safe base64 strings than manual base64 encoding followed by regex replacements (measured up to ~8.4x speedup and creates less GC garbage).
**Action:** Always use native `.toString('base64url')` when URL-safe base64 encoding is needed in Node.js backend/bot code.
