...
## 2024-06-25 - Native Base64Url vs Regex Replacements
**Learning:** Generating base64url strings in Node.js using `.toString('base64').replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_')` is significantly slower (~2x) and more memory intensive than using native `.toString('base64url')` which handles the alphabet translation and padding stripping internally.
**Action:** Always prefer native `.toString('base64url')` encoding in Node.js environments instead of manually transforming base64 strings with regexes, as it is faster and more readable.
