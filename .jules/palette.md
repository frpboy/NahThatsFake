...
## 2026-06-02 - Consolidating Screen Reader Announcements for Interactive Cards
**Learning:** For complex interactive list items or cards (like recent checks), applying individual ARIA attributes to inner structural elements can lead to fragmented and confusing screen reader readouts. It's much more intuitive to consolidate the entire announcement.
**Action:** Apply a single, descriptive `aria-label` to the parent container that summarizes all relevant information. Then, set `aria-hidden="true"` on all inner visual/structural child elements so they are ignored by the screen reader, ensuring a single cohesive readout when the user focuses on the card.
