...
app.get('/api/user/checks', validateTelegramData, async (req, res) => {
    ...
    try {
      // ⚡ Bolt: Combine user and checks queries into a single round-trip via resource embedding
      ...
      if (dbError && dbError.code === 'PGRST116') return res.status(404).json({ error: 'User not found' });
      if (!user) return res.status(404).json({ error: 'User not found' });
