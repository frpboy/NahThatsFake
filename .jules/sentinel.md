...
## 2025-02-23 - Insecure Webhook Signature Validation via JSON.stringify
**Vulnerability:** The Razorpay webhook endpoint (`/api/payment/razorpay-webhook`) computed its HMAC signature using `JSON.stringify(req.body)`. This is highly problematic because the Express body parser and `JSON.stringify` may alter formatting (such as whitespace and key order) from the original raw request payload, causing valid signatures to fail or allowing attackers to bypass validation using crafted JSON payloads.
**Learning:** Webhook HMAC signatures must always be calculated against the exact raw bytes sent by the external service, not a re-serialized representation.
**Prevention:** Always capture the raw request body using Express middleware (e.g., `app.use(express.json({ verify: (req, res, buf) => { req.rawBody = buf; } }))`) and use `req.rawBody` as the input for HMAC signature calculations.
