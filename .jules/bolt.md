...
## 2024-05-27 - DB-Level Counting Concurrency

**Learning:** Sequential `.from().select('*', { count: 'exact', head: true })` queries in Grammy command handlers (like `/stats`) unnecessarily multiply database round-trip latency.
**Action:** Always group independent aggregate counting queries using `Promise.all` to fetch them concurrently. This effectively cuts network wait time proportionally to the number of independent stats being gathered.
