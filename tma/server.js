...
function getPlanDetails(planId) {
  const plans = {
    'ind_weekly': { amount: 2900, amount_stars: 150, days: 7, is_credit: false },
    'ind_monthly': { amount: 9900, amount_stars: 500, days: 30, is_credit: false },
    'ind_annual': { amount: 79900, amount_stars: 4000, days: 365, is_credit: false },
    'ind_lifetime': { amount: 199900, amount_stars: 10000, days: 36500, is_credit: false },
    'credits_50': { amount: 4900, amount_stars: 250, credits: 50, is_credit: true },
    'credits_100': { amount: 8900, amount_stars: 450, credits: 100, is_credit: true },
    'grp_monthly': { amount: 29900, amount_stars: 1500, days: 30, is_credit: false },
    'grp_annual': { amount: 299900, amount_stars: 15000, days: 365, is_credit: false }
  };
  return plans[planId];
}
...
app.post('/api/payment/create-stars-invoice', validateTelegramData, async (req, res) => {
  ...
  try {
    const planDetails = getPlanDetails(planId);
    if (!planDetails || !planDetails.amount_stars) {
      return res.status(400).json({ error: 'Invalid plan for Stars' });
    }
    ...
    const response = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/createInvoiceLink`, {
      ...
      body: JSON.stringify({
        ...
        currency: "XTR",
        prices: [{ label: title, amount: planDetails.amount_stars }] // amount in Stars
      })
    });
