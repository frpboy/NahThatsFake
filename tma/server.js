...
app.get('/api/user/checks', validateTelegramData, async (req, res) => {
    ...
    try {
      // ⚡ Bolt: Combine user lookup and check fetching into a single query via parent-to-child
      // resource embedding. This saves a database round-trip while `.single()` correctly maintains
      // the strict 404 behavior if the user doesn't exist.
      const { data: user } = await supabase
        .from('users')
        .select('id, checks(*)')
        .eq('telegram_user_id', userId.toString())
