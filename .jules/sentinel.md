...
## 2024-05-31 - [Webhook Signature Verification Bypass & DoS]
**Vulnerability:** Express `JSON.stringify(req.body)` was used for HMAC signature generation, which modifies formatting/ordering and invalidates payload verification. Furthermore, an absent webhook secret could cause a `TypeError` in `crypto.createHmac`, leading to unhandled promise rejections and potential Denial-of-Service.
**Learning:** HMAC calculation requires the absolute original byte payload. Cryptographic API calls must defensively check for the presence of required key variables before execution.
**Prevention:** Always buffer `req.rawBody` directly in the `express.json` middleware via the `verify` callback. Explicitly validate environment variables before passing them into `crypto` functions.
