...
## 2024-11-06 - Secure Webhook Signature Verification and Cryptographic Safety
**Vulnerability:** The Razorpay webhook handler was using `JSON.stringify(req.body)` to compute the HMAC signature, which alters formatting and key order, leading to failed verifications. Furthermore, missing environment variables passed to `crypto.createHmac()` would throw `TypeError` exceptions, causing Unhandled Promise Rejections and server crashes (DoS).
**Learning:** Always compute signatures against the exact raw request body captured via `express.json({ verify: (req, res, buf) => { req.rawBody = buf } })`. In addition, cryptographic functions must only be called after explicitly verifying the presence of secrets to avoid application crashes.
**Prevention:** Implement checks for secret configuration before any `crypto.createHmac()` call, ensure proper fallback on `update()` arguments to prevent errors on undefined bodies, and strictly use `req.rawBody` for webhook verifications instead of reconstructing the payload.
