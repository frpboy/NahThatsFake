...
function renderChecks() {
    ...
    const html = checks.map(check => {
        ...
        return `
            <div class="check-item" role="button" tabindex="0" aria-label="${check.check_type === 'image' ? 'Image Analysis' : 'Link Analysis'} on ${date}. Risk level: ${check.risk_level || 'UNKNOWN'}, Score: ${scorePercent}%" onclick="viewCheckDetails('${check.id}')" onkeydown="if(event.key === 'Enter' || event.key === ' ') { event.preventDefault(); viewCheckDetails('${check.id}'); }">
