...
## 2024-05-29 - Complex Interactive Lists Announcements
**Learning:** Complex interactive list items like `.check-item` cause fragmented and confusing screen reader announcements when they contain multiple elements with separate text.
**Action:** Consolidate screen reader readouts by applying a single descriptive `aria-label` to the parent `.check-item` container and setting `aria-hidden="true"` on all inner visual/structural child elements. This provides a clear, unified announcement.
