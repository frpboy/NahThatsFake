...
## 2025-02-18 - Native Base64Url Encoding
**Learning:** In Node.js environments (v14.18+), using `Buffer.from(data).toString('base64url')` is significantly more efficient for generating URL-safe base64 strings than manual base64 encoding followed by multiple regex replacements (e.g. `.replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_')`).
**Action:** Always prefer native `base64url` encoding string generation rather than chaining Regex replacements, this provides a noticeable speedup and consumes less memory.
