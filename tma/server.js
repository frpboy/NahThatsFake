...
function getPlanDetails(planId) {
  const plans = {
    ...
    'ind_lifetime': { amount: 199900, stars: 0, days: 36500, is_credit: false },
    'credits_50': { amount: 4900, stars: 250, credits: 50, is_credit: true },
    'credits_100': { amount: 8900, stars: 450, credits: 100, is_credit: true },
    'grp_monthly': { amount: 29900, stars: 0, days: 30, is_credit: false },
    'grp_annual': { amount: 299900, stars: 0, days: 365, is_credit: false }
  };
  return plans[planId];
}
...
app.post('/api/payment/create-razorpay-order', validateTelegramData, async (req, res) => {
  const { planId } = req.body; // 🛡️ Sentinel: Ignore client-provided amount
  const userId = req.telegramUser ? req.telegramUser.id : null;
  if (!userId) return res.status(400).json({ error: 'Missing userId' });
  ...
  try {
    const options = {
      amount: planDetails.amount, // 🛡️ Sentinel: Use authoritative server-side amount
      currency: "INR",
...
app.post('/api/payment/create-stars-invoice', validateTelegramData, async (req, res) => {
  ...
  const planDetails = getPlanDetails(planId);
  if (!planDetails || !planDetails.stars) return res.status(400).json({ error: 'Invalid planId or stars not supported' });
  ...
  try {
    ...
    const response = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/createInvoiceLink`, {
      ...
      body: JSON.stringify({
        ...
        currency: "XTR",
        prices: [{ label: title, amount: planDetails.stars }] // 🛡️ Sentinel: Use authoritative server-side stars amount
      })
    });
