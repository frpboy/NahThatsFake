...
## 2024-05-24 - Consolidated screen reader readouts for complex list items
**Learning:** For complex interactive list items or cards (like the recent checks list), screen readers will fragment the announcement of inner text elements, making the experience disjointed.
**Action:** Consolidate screen reader readouts by applying a single descriptive `aria-label` to the parent container and setting `aria-hidden="true"` on all inner visual/structural child elements to prevent fragmented announcements.
