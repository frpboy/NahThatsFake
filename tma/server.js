...
app.post('/api/payment/create-razorpay-order', validateTelegramData, async (req, res) => {
  ...
  try {
    const options = {
      amount: planDetails.amount, // 🛡️ Sentinel: Enforce server-side pricing to prevent IDOR
      currency: "INR",
...
app.post('/api/payment/create-stars-invoice', validateTelegramData, async (req, res) => {
  ...
  const planDetails = getPlanDetails(planId);
  if (!planDetails || !planDetails.stars) return res.status(400).json({ error: 'Invalid planId or stars pricing' });
  ...
  try {
    ...
    const response = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/createInvoiceLink`, {
      ...
      body: JSON.stringify({
        ...
        currency: "XTR",
        prices: [{ label: title, amount: planDetails.stars }] // 🛡️ Sentinel: Enforce server-side stars pricing
      })
    });
