...
## 2024-06-13 - Optimize Base64 URL Encoding
**Learning:** In Node.js (v14.18+), replacing manual base64 encoding and string regex replacements (`.replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_')`) with `Buffer.from(data).toString('base64url')` provides a significant performance boost (~3x faster in local tests) and generates less garbage collection overhead.
**Action:** Always prefer native Node.js buffer encodings (`base64url`) over manual string manipulation for cryptographic or encoding-heavy operations.
