...
bot.use(async (ctx, next) => {
  ...
  // ⚡ Bolt: Fetch role and ban status in a single query to avoid redundant DB calls on every message
  const { data } = await supabase
    .from('users')
  ...
  // Cache role for downstream middleware
  if (!('state' in ctx)) (ctx as any).state = {};
  (ctx as any).state.userRole = data?.role || 'user';

  // Bypass for admins/owners
  if (data?.role === 'admin' || data?.role === 'owner') return next();
...
bot.use(async (ctx, next) => {
  ...
  // ⚡ Bolt: Check cached role first to avoid an extra DB query per message
  const cachedRole = (ctx as any).state?.userRole;
  const isOwnerStatus = cachedRole ? cachedRole === 'owner' : await isOwner(fromTelegramUserId);
