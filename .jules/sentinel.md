...
## $(date +%Y-%m-%d) - Prevent Idempotency Bypass and Silent Failures in Supabase
**Vulnerability:** Supabase insert/update operations do not throw exceptions by default on failure, leading to potential silent failures where payments appear verified but no credits are added, or worse, idempotency mechanisms can be bypassed if the update fails but the endpoint returns success.
**Learning:** The default behavior of the Supabase JS client requires explicit extraction and checking of the `error` object from the returned tuple.
**Prevention:** Always extract and handle the `error` object explicitly on database writes, especially in critical financial or access-control paths.
