...
function getPlanDetails(planId) {
  const plans = {
    ...
    'ind_lifetime': { amount: 199900, stars: null, days: 36500, is_credit: false },
    'credits_50': { amount: 4900, stars: 250, credits: 50, is_credit: true },
    'credits_100': { amount: 8900, stars: 450, credits: 100, is_credit: true },
    'grp_monthly': { amount: 29900, stars: null, days: 30, is_credit: false },
    'grp_annual': { amount: 299900, stars: null, days: 365, is_credit: false }
  };
  return plans[planId];
}
...
app.post('/api/payment/create-stars-invoice', validateTelegramData, async (req, res) => {
  ...
  if (!planDetails || !planDetails.stars) {
    return res.status(400).json({ error: 'Invalid planId or stars pricing not available' });
  }
