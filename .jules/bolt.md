...
## 2024-07-20 - DB-Level Joining via Parent-to-Child Resource Embedding

**Learning:** When fetching nested resources sequentially (e.g., querying `users` and then their `checks`), doing multiple `.select()` database queries introduces N+1 performance bottlenecks due to network round trips. However, using `.inner` joins can silently alter expected `.single()` behavior (e.g. failing to 404 if the parent is missing).
**Action:** Use parent-to-child resource embedding queries (`supabase.from('users').select('id, checks(*)')`) combined with `foreignTable` options on modifiers like `.order()` and `.limit()`. This approach combines sequential database requests into a single round-trip without changing missing-parent behaviour on `.single()`.
