...
## 2024-05-17 - Native Base64URL Encoding
**Learning:** Using `Buffer.from(data).toString('base64url')` is significantly more efficient for generating URL-safe base64 strings than manual base64 encoding followed by regex replacements (measured ~2.2x speedup in Node.js environments).
**Action:** Use native `base64url` instead of regex replacements for URL-safe base64 strings in Node.js.
