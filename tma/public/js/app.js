...
function renderChecks() {
    ...
    const html = checks.map(check => {
        ...
        const label = `${check.check_type === 'image' ? 'Image Analysis' : 'Link Analysis'} from ${date}. Result: ${check.risk_level || 'UNKNOWN'} risk at ${scorePercent} percent.`;
        return `
            <div class="check-item" role="button" tabindex="0" aria-label="${label}" onclick="viewCheckDetails('${check.id}')" onkeydown="if(event.key === 'Enter' || event.key === ' ') { event.preventDefault(); viewCheckDetails('${check.id}'); }">
