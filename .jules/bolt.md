...
## 2025-07-02 - Combine Parent and Child Queries Using Supabase Resource Embedding
**Learning:** Sequential parent and child database queries (e.g., getting a user, then getting their checks) are a common anti-pattern that creates unnecessary network latency (N+1-like issue). Using Supabase parent-to-child resource embeddings (`select('id, checks(*)')`) eliminates the extra round trip. When querying embedded children, options like `order` and `limit` *must* use the `referencedTable` key (e.g. `limit(10, { referencedTable: 'checks' })`) to function correctly in v2. Using `inner` joins conversely breaks `404` behavior by returning an empty array if the child is missing.
**Action:** When a parent item is queried immediately followed by a dependent query for its children, combine them into one request using `select('parent_field, child_table(*)')`. Apply pagination and ordering with the `referencedTable` option to ensure correct execution.
