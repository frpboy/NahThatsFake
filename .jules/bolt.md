...
## 2024-06-04 - Native base64url String Encoding
**Learning:** In Node.js environments (v14.18+), using `Buffer.from(data).toString('base64url')` is significantly more efficient for generating URL-safe base64 strings than manual base64 encoding followed by regex string replacements (`.replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_')`). It uses native C++ bindings which is up to ~2.3x faster and creates less garbage collection overhead.
**Action:** Always use the native `'base64url'` encoding string in `Buffer.toString()` when dealing with URL identifiers or cryptographic signatures instead of chained regex replacements.
