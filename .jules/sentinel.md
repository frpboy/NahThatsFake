...
## 2024-05-15 - [CRITICAL] Price Manipulation in Payment Endpoints
**Vulnerability:** Client-side price manipulation. The backend endpoints (`/api/payment/create-razorpay-order` and `/api/payment/create-stars-invoice`) were trusting the `amount` passed in the request body from the client when initiating payment requests.
**Learning:** Payment systems must never trust client-provided pricing parameters. Even if the UI shows the correct price, malicious actors can bypass the UI and send arbitrary amounts directly to the API, allowing them to purchase plans for practically nothing.
**Prevention:** Always enforce authoritative server-side price lookups based on the requested plan/item ID. Update the backend logic to strictly use internal plan configurations (e.g., `getPlanDetails(planId)`) for all financial operations, ignoring client assertions of cost.
