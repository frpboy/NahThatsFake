...
## 2024-05-18 - Prevent Replay Attacks on Telegram Mini App Authentication
**Vulnerability:** The TMA backend authentication middleware (`validateTelegramData`) verified the HMAC signature but failed to validate the `auth_date` parameter within the initData payload, allowing mathematically sound but completely expired payloads to be reused indefinitely (replay attack). Furthermore, missing `BOT_TOKEN` configs caused a fallback to a hardcoded string, completely compromising the HMAC secret.
**Learning:** Telegram explicitly requires checking the `auth_date` against the current timestamp to ensure the data is fresh (usually within 24 hours), as `initData` is generated once on client load and can be intercepted.
**Prevention:** Always parse and validate `auth_date` (allowing max 24h drift) in any Telegram WebApp authentication flow, and return strict 500 errors instead of weak fallback defaults when critical cryptographic environment variables are missing.
