...
app.post('/api/payment/verify-razorpay', validateTelegramData, async (req, res) => {
  ...
  if (!process.env.RAZORPAY_KEY_SECRET) {
    return res.status(500).json({ error: 'Internal server error: Razorpay secret not configured' });
  }
...
app.post('/api/payment/razorpay-webhook', async (req, res) => {
  ...
  if (!webhookSecret) return res.status(500).send('Internal server error: Webhook secret not configured');
