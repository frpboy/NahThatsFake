...
## 2026-06-10 - Consolidate Complex Interactive Elements for Screen Readers
**Learning:** Fragmented readouts occur when screen readers encounter complex interactive cards containing multiple visual elements, even if semantic.
**Action:** Consolidate announcements by applying a single descriptive `aria-label` to the parent container and setting `aria-hidden="true"` on all inner visual or structural child elements.
