...
## 2026-06-20 - Consolidate screen reader readouts for complex vanilla JS list items
**Learning:** For interactive list items built with vanilla JS and multiple nested DOM elements, screen readers may read each element in a fragmented manner, creating a poor user experience.
**Action:** When creating complex interactive components (like `.check-item`), apply a single descriptive `aria-label` to the parent container and set `aria-hidden="true"` on all inner child elements. This provides a clear, consolidated announcement for assistive technologies.
