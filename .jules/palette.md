...
## 2026-05-20 - Consolidate Screen Reader Readouts
**Learning:** For complex interactive list items or cards, screen readers can produce fragmented announcements if each inner element is read separately.
**Action:** Always consolidate screen reader readouts by applying a single descriptive `aria-label` to the parent container and setting `aria-hidden="true"` on all inner visual/structural child elements.
