...
const allowedOrigins = (
  process.env.ALLOWED_ORIGINS || "https://nahthatsfake.vercel.app"
).split(",");
app.use(
  cors({
...
const validateTelegramData = (req, res, next) => {
  ...
  if (!initData && process.env.NODE_ENV === "development") {
    console.warn(
      "Skipping Telegram validation in development (missing header)",
    );
    return next();
  }
  ...
  if (!initData) {
    return res
      .status(401)
      .json({ error: "Missing X-Telegram-Init-Data header" });
  }
  ...
  if (!process.env.BOT_TOKEN) {
    console.error(
      "CRITICAL: BOT_TOKEN is not configured for Telegram validation",
    );
    return res
      .status(500)
      .json({ error: "Internal server error: Authentication misconfigured" });
  }
...
app.get("/api/admin/stats", validateTelegramData, async (req, res) => {
  try {
    ...
    const [
      ...
      supabase
        .from("payments")
        .select("amount_inr, amount_stars")
        .eq("status", "success"),
    ]);

    const revenueInr =
      payments?.reduce((acc, curr) => acc + (curr.amount_inr || 0), 0) / 100 ||
      0;
    const revenueStars =
      payments?.reduce((acc, curr) => acc + (curr.amount_stars || 0), 0) || 0;

    if (usersError || bannedError || checksError || paymentsError)
...
app.get("/api/user/checks", validateTelegramData, async (req, res) => {
  ...
  try {
    // ⚡ Bolt: Fetch user and their latest checks in a single query to reduce DB round-trips
    const { data: user, error } = await supabase
      .from("users")
      .select("id, checks(*)")
      .eq("telegram_user_id", userId.toString())
      .order("created_at", { referencedTable: "checks", ascending: false })
    ...
    if (timingSafeCompare(generated_signature, razorpay_signature)) {
      ...
      try {
        ...
        await supabase.from("payments").insert({
          ...
          premium_until: planDetails.is_credit
            ? null
            : new Date(
                Date.now() + planDetails.days * 24 * 60 * 60 * 1000,
              ).toISOString(),
        });
        ...
        if (planDetails.is_credit) {
          ...
          await supabase
            ...
            .update({
              permanent_credits:
                (user.permanent_credits || 0) + planDetails.credits,
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (planId === "ind_weekly") {
        title = "Weekly Pass";
        description = "7 Days Premium";
      }
      if (planId === "ind_monthly") {
        title = "Monthly Premium";
        description = "30 Days Premium";
      }
      if (planId === "ind_annual") {
        title = "Annual Premium";
        description = "365 Days Premium";
      }
      if (planId === "credits_50") {
        title = "50 Credits";
        description = "50 Permanent Checks";
      }
      if (planId === "credits_100") {
        title = "100 Credits";
        description = "100 Permanent Checks";
      }
        ...
        if (telegramUserId && planId) {
          ...
          if (user) {
            ...
            if (planDetails) {
              ...
              console.log(
                `Webhook: Fulfilled ${planId} for user ${telegramUserId}`,
              );
            }
          }
        }
