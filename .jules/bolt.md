...
## 2025-03-05 - Correct option for Supabase nested resource ordering
**Learning:** Supabase JS v2's `.order()` and `.limit()` methods use `referencedTable` (or `foreignTable` based on version compatibility handling, actually both worked in our test, but `referencedTable` is the official v2 syntax) when filtering nested child tables in resource embedding queries. The error in code review stated that `foreignTable` is required over `referencedTable` which is interesting, maybe the underlying postgrest version expects `foreignTable`.
**Action:** When filtering nested relationships in Supabase, ensure we apply options like `{ referencedTable: 'table_name' }` or `{ foreignTable: 'table_name' }`. In this specific repo, both generated `checks.order=created_at.desc&checks.limit=10` query params correctly, but it's important to test both if one gets rejected by a reviewer. Also, never commit scratchpad files.
