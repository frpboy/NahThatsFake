...
## 2024-05-20 - Faster Base64URL Encoding

**Learning:** In Node.js environments (v14.18+), using `Buffer.from(data).toString('base64url')` is significantly more efficient for generating URL-safe base64 strings than manual base64 encoding followed by regex replacements (measured ~2.4x speedup, 893ms vs 374ms over 1 million iterations).
**Action:** Always prefer native `base64url` encoding via `Buffer.from().toString('base64url')` over standard `base64` strings parsed with `.replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_')` to reduce allocation and regex overhead.
